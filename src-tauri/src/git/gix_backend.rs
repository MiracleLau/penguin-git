...
impl GitBackend for GixBackend {
    fn get_status(&self, path: &str) -> Result<Vec<FileStatus>, String> {
        ...
        for line in output.lines() {
            if line.is_empty() || line.as_bytes().len() < 3 { continue; }
            let staged = line.as_bytes()[0] != b' ';
            let file = &line[3..];
            let status = match (line.as_bytes()[0], line.as_bytes()[1]) {
                (b'?', _) | (_, b'?') => "new", (b'M', _) | (_, b'M') => "modified",
                (b'D', _) | (_, b'D') => "deleted", (b'R', _) | (_, b'R') => "renamed",
                (b'A', _) | (_, b'A') => "new", _ => "modified",
            };
            let display_path = if let Some(idx) = file.find(" -> ") { &file[idx + 4..] } else { file };
            files.push(FileStatus {
                path: display_path.trim().to_string(), status: status.to_string(),
    ...
    fn stage_all(&self, path: &str) -> Result<(), String> { cli(&["add", "-A"], path).map(|_| ()) }
    ...
    fn commit_with_identity(&self, path: &str, message: &str, name: &str, email: &str) -> Result<CommitInfo, String> {
        self.stage_all(path)?;
        let output = cli(&["-c", &format!("user.name={}", name), "-c", &format!("user.email={}", email), "commit", "-m", message], path)?;
        let hash = output.lines().find(|l| l.contains(']'))
            .and_then(|l| l.split(']').next()?.split_whitespace().last().map(|s| s.to_string()))
            .unwrap_or_default();
        Ok(CommitInfo {
            short_hash: hash.chars().take(7).collect(), hash, message: message.to_string(),
    ...
    fn get_log(&self, path: &str, limit: u32) -> Result<Vec<CommitInfo>, String> {
        let output = cli(&["log", "--format=%H|%an|%at|%s", &format!("--max-count={}", limit)], path)?;
        ...
        for line in output.lines() {
            ...
            commits.push(CommitInfo {
                hash: parts[0].to_string(), short_hash: parts[0].chars().take(7).collect(),
                author: parts[1].to_string(), time: parts[2].parse().unwrap_or(0),
    ...
    fn get_ahead_behind(&self, path: &str) -> Result<(usize, usize), String> {
        ...
        let tracking = match cli(&["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], path) {
            Ok(t) => t.trim().to_string(),
            Err(_) => return Ok((0, 0)),
        };
        let ahead = cli(&["rev-list", "--count", &format!("{}..{}", tracking, branch)], path)?;
        let behind = cli(&["rev-list", "--count", &format!("{}..{}", branch, tracking)], path)?;
        Ok((ahead.trim().parse().unwrap_or(0), behind.trim().parse().unwrap_or(0)))
    }
    ...
    fn revert_to_commit(&self, path: &str, hash: &str, message: &str, name: &str, email: &str) -> Result<(), String> {
        ...
        cli(&["-c", &format!("user.name={}", name), "-c", &format!("user.email={}", email), "commit", "-m", message], path).map(|_| ())
    }
